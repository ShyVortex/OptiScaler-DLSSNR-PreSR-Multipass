...
Image modelAnswer(Image image, bool hdr, float gain, float colour)
{
    for (auto& p : image.pixels)
    {
        ...
        p.r = gain * (p.r + colour * y);
        p.b = gain * (p.b - colour * y);
...
try
{
    ...
    bd.ByteWidth = sizeof(DlssNrConstants);
    ...
    auto run = [&](const DlssNrConstants& c, const Image& source, const Image& model, const Image& original)
    {
        ...
        ctx->CSSetUnorderedAccessViews(0, 2, outputs, nullptr);
        ...
        for (unsigned y = 0; y < c.Height; ++y)
            memcpy(result.pixels.data() + y * c.Width, static_cast<char*>(mapped.pData) + y * mapped.RowPitch,
    ...
    for (unsigned y = 0; y < 8; ++y)
        for (unsigned x = 0; x < 100; ++x)
            original.pixels[y * 100 + x] =
                x < 51 ? Pixel { .024f, .018f, .012f, .4f } : Pixel { .65f, .36f, .24f, .8f };
    unsigned cases = 0;
    float worst = 0, legacyHalo = 0;
    for (unsigned hdr : { 0u, 1u })
        for (unsigned curve = 0; curve < 5; ++curve)
        {
            DlssNrConstants c {};
            c.Width = 100;
            c.Height = 8;
            ...
            c.Passthrough = !hdr;
            c.WhitePoint = 1;
            c.ReversibleMode = curve;
            c.MaxRatio = 8;
            c.TransferStrength = c.ColourStrength = 1;
            c.ApplyModel = 1;
            c.CompareZoom = 1;
            const auto full = run(c, original, original, original);
            for (float gain : { .75f, 1.0f, 1.15f })
                for (float colour : { 0.0f, .05f })
                {
                    const auto nativeAnswer = modelAnswer(full, hdr != 0, gain, colour);
                    c.Mode = DlssNrMode_Resolve;
                    c.Transfer = 0;
                    ...
                    for (unsigned percent : { 25u, 65u, 99u, 100u, 101u })
                    {
                        auto work = c;
                        work.Width = percent;
                        work.Height = std::max(2u, 8 * percent / 100);
                        ...
                        if (percent < 100)
                        {
                            float error = difference(reconstructed, reference);
                            worst = std::max(worst, error);
                            if (error > .0004f)
                                std::printf("hdr=%u curve=%u gain=%g colour=%g scale=%u error=%g\n", hdr, curve, gain,
                                            colour, percent, error);
                            expect(error < .0004f,
                                   "Separated resize changed a uniform lighting/chroma edit at the hair/skin edge");
                            c.Transfer = 1;
                            const auto legacy = run(c, proxy, answer, original);
                            legacyHalo = std::max(legacyHalo, difference(legacy, reference));
                            c.Transfer = 3;
                            c.ApplyModel = 0;
                            expect(difference(run(c, proxy, answer, original), original) < 1e-6f,
                                   "Bypass changed the source frame");
                            c.ApplyModel = 1;
                            ...
                            c.Transfer = 4;
                            expect(difference(run(c, field, enlarged, original), reference) < .0005f,
                                   "Private carrier reconstruction mismatch");
                            std::fill(enlarged.pixels.begin(), enlarged.pixels.end(), Pixel { 2, -1, 3, 1 });
                            expect(difference(run(c, field, enlarged, original), reference) < .0005f,
                                   "Private carrier range guard failed");
                        }
                        else
                        {
                            c.Transfer = 0;
                            expect(difference(reconstructed, run(c, proxy, answer, original)) < 1e-6f,
                                   "Native/supersampled path changed");
                        }
                        ++cases;
                    }
                }
    ...
    DlssNrConstants black {};
    black.Mode = DlssNrMode_Resolve;
    black.Width = 100;
    black.Height = 8;
    black.Passthrough = black.ApplyModel = 1;
    black.ReversibleMode = 2;
    black.Transfer = 3;
