...
try
{
    ...
    device->CreateUnorderedAccessView(texture.Get(), nullptr, nullptr,
    ...
    auto clear = [&](float value)
    {
        ...
        cmd->ClearUnorderedAccessViewFloat(descriptors->GetGPUDescriptorHandleForHeapStart(),
                                           descriptors->GetCPUDescriptorHandleForHeapStart(), texture.Get(), color, 0,
                                           nullptr);
    };
    const auto root =
    ...
    expect(frame->Init(device.Get()), "capture init");
    frame->directory = root / "submitted";
    lifetime.Record(cmd.Get());
    clear(.25f);
    frame->Copy(cmd.Get(), device.Get(), "before_nr", texture.Get(), D3D12_RESOURCE_STATE_UNORDERED_ACCESS);
    clear(.75f);
    frame->Copy(cmd.Get(), device.Get(), "after_nr", texture.Get(), D3D12_RESOURCE_STATE_UNORDERED_ACCESS);
    clear(.5f);
    frame->Copy(cmd.Get(), device.Get(), "after_rr", texture.Get(), D3D12_RESOURCE_STATE_UNORDERED_ACCESS);
    frame->End(cmd.Get());
    check(cmd->Close());
    bool written = false;
    lifetime.Retire(
    ...
    lifetime.Submitted(queue.Get(), 1, lists);
    lifetime.Collect();
    ...
    expect(WaitForSingleObject(event, 10000) == WAIT_OBJECT_0, "GPU timeout");
    CloseHandle(event);
    check(allocator->Reset());
    check(cmd->Reset(allocator.Get(), nullptr));
    lifetime.ResetRecording(cmd.Get());
    expect(written && lifetime.Idle(), "completed capture was not written/reclaimed");
    const char* names[] { "before_nr", "after_nr", "after_rr" };
    const float values[] { .25f, .75f, .5f };
    for (int i = 0; i < 3; ++i)
    {
    ...
    expect(discarded->Init(device.Get()), "discard init");
    discarded->directory = root / "discarded";
    lifetime.Record(cmd.Get());
    discarded->Copy(cmd.Get(), device.Get(), "image", texture.Get(), D3D12_RESOURCE_STATE_UNORDERED_ACCESS);
    discarded->End(cmd.Get());
    check(cmd->Close());
    bool rejected = false;
    lifetime.Retire(
        [&]
        {
            rejected = !discarded->Write();
            delete discarded;
        });
    check(allocator->Reset());
    check(cmd->Reset(allocator.Get(), nullptr));
