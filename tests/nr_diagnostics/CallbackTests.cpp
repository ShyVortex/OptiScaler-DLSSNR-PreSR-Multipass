...
int main()
{
    ...
    assert(forwarded == 1 && diagnosticLines.empty());
    {
        ...
        std::thread worker([&] { callback("worker", NVSDK_NGX_LOGGING_LEVEL_VERBOSE, (NVSDK_NGX_Feature) 18); });
        worker.join();
        assert(diagnosticLines.size() == 1 && forwarded == 1);
        for (int i = 0; i < 600; ++i)
            callback("bounded", NVSDK_NGX_LOGGING_LEVEL_VERBOSE, (NVSDK_NGX_Feature) 18);
