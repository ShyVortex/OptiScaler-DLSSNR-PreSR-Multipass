...
try
{
    ...
    if (errors)
        std::fputs((const char*) errors->GetBufferPointer(), stderr);
    check(compiled);
                   ...
                   const std::vector<Pixel>& history, const std::vector<Pixel>& motion)
    {
        ...
        if (c.Mode == 5)
        {
            ...
            result.assign(first, first + c.Width);
        ctx->Unmap(readback.Get(), 0);
        return result;
    };
    // Independent reference points: ST.2084 code values for 0, 80, 1000 and 10000 nits.
    std::vector<Pixel> pq = { { 0, 0, 0, 0.1f },
                              { 0.4858567654f, 0.4858567654f, 0.4858567654f, 0.2f },
                              { 0.7518270962f, 0.7518270962f, 0.7518270962f, 0.3f },
                              { 1, 1, 1, 0.4f },
                              { 0.7f, 0.1f, 0.2f, 0.5f },
                              { 0.2f, 0.8f, 0.1f, 0.6f },
                              { 0.1f, 0.2f, 0.9f, 0.7f } };
    DlssNrConstants c {};
    c.Width = (unsigned) pq.size();
    c.Height = 1;
    ...
    for (unsigned i = 0; i < pq.size(); ++i)
    {
        expect(std::abs(encoded[i].r - pq[i].r) < 0.0002f && std::abs(encoded[i].g - pq[i].g) < 0.0002f &&
                   std::abs(encoded[i].b - pq[i].b) < 0.0002f,
               "HDR10 round trip changed colour");
    ...
    c.Width = 3;
    c.WhitePoint = 1;
    c.TransferStrength = 1;
    c.MaxRatio = 2;
    const std::vector<Pixel> clean(3, { 1, 1, 1, 1 });
    const std::vector<Pixel> carrier = { { 0.625f, 0.625f, 0.625f, 1 },
                                         { 0.375f, 0.375f, 0.375f, 1 },
                                         { 0.5f, 0.5f, 0.5f, 1 } };
    ...
    c.Mode = 3;
    final.assign(3, { 12.5f, 12.5f, 12.5f, 0.375f });
    ...
    auto invalid = carrier;
    invalid[0].r = std::numeric_limits<float>::quiet_NaN();
    ...
    for (auto& p : darkEdits)
        p.r = p.g = p.b = 0.5f + std::log2(0.8f) / 8.0f;
    ...
    for (const auto& bin : flatCurve)
        expect(bin.b == 0, "Flat scene invented a response");
    c.Mode = 8;
    c.Width = samples;
    ...
    auto overlay = display;
    overlay[samples / 2] = { 100, 100, 100, 0.6f };
    c.Mode = 3;
    auto overlayOld = run(c, overlay, scene, edits, curve);
    c.Mode = 8;
    auto overlayNew = run(c, overlay, scene, edits, curve);
    ...
    for (auto& p : historyCurve)
        p.r += 0.1f;
    c.Mode = 6;
    c.Width = kDlssNrHdrCurveBins;
    c.WhitePoint = 1;
    c.DebugView = 1;
    c.ColourStrength = 0.35f;
    ...
    for (unsigned i = 0; i < samples; ++i)
        if (scene[i].r >= 0.1f && scene[i].r <= 64.0f)
            pqError += std::abs(matchedPq[i].r - expectedPq[i].r);
    ...
    std::puts(
        "PASS: pre-SR residual transfer in SDR/scRGB/HDR10, signed edits, neutral bypass, exposure, invalid input");
    return 0;
}
