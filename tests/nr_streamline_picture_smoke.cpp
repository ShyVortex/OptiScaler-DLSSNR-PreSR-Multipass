...
static UINT Index(IDXGISwapChain*, bool& handled)
{
    handled = handledIndex;
    return appIndex;
}
...
try
{
    ...
    for (UINT i = 0; i < 4; ++i)
    {
        Check(device->CreateCommittedResource(&heap, D3D12_HEAP_FLAG_NONE, &td, D3D12_RESOURCE_STATE_COMMON, nullptr,
                                              IID_PPV_ARGS(&textures[i])));
        device->CreateRenderTargetView(textures[i].Get(), nullptr, rtv(i));
        if (i < 3)
            appBuffers[i] = textures[i].Get();
    }
    ...
    Check(device->CreateCommittedResource(&heap, D3D12_HEAP_FLAG_NONE, &bd, D3D12_RESOURCE_STATE_COPY_DEST, nullptr,
    ...
    auto barrier = [&](ID3D12Resource* res, D3D12_RESOURCE_STATES from, D3D12_RESOURCE_STATES to)
    {
        ...
        b.Transition = { res, D3D12_RESOURCE_BARRIER_ALL_SUBRESOURCES, from, to };
    ...
    for (UINT frame = 0; frame < 12; ++frame)
    {
        ...
        Check(queue->Signal(fence.Get(), frame + 1));
        Check(fence->SetEventOnCompletion(frame + 1, event));
        Expect(WaitForSingleObject(event, 5000) == WAIT_OBJECT_0, "GPU timeout");
        unsigned char* bytes = nullptr;
        Check(readback->Map(0, nullptr, reinterpret_cast<void**>(&bytes)));
        const bool visible = bytes[1] == 255;
    ...
    puts("PASS native FG handoff: rotating app buffers; old edit overwritten, pre-FG edit retained; no display "
         "fallback");
    return 0;
}
