...
{
    if (r != VK_SUCCESS)
        throw std::runtime_error("Vulkan result " + std::to_string(r));
}
int main(int argc, char** argv)
try
{
    if (argc != 2)
        throw std::runtime_error("Pass the production DlssNr_Shader_Vk.spv path");
    ...
    file.seekg(0);
    file.read((char*) code.data(), bytes);
    VkApplicationInfo app { VK_STRUCTURE_TYPE_APPLICATION_INFO };
    app.pApplicationName = "NR Vulkan shader smoke";
    ...
    vkGetPhysicalDeviceFeatures(pd, &available);
    ...
    auto allocate = [&](VkMemoryRequirements req, VkMemoryPropertyFlags flags)
    {
        ...
        for (ai.memoryTypeIndex = 0; ai.memoryTypeIndex < memory.memoryTypeCount; ++ai.memoryTypeIndex)
            if ((req.memoryTypeBits & (1u << ai.memoryTypeIndex)) &&
                (memory.memoryTypes[ai.memoryTypeIndex].propertyFlags & flags) == flags)
                break;
        if (ai.memoryTypeIndex == memory.memoryTypeCount)
            throw std::runtime_error("No suitable memory type");
        VkDeviceMemory m {};
        check(vkAllocateMemory(device, &ai, nullptr, &m));
        return m;
    };
    ...
    for (auto& image : images)
    {
        ...
        ci.extent = { allocationWidth, allocationHeight, 1 };
    ...
    constants.ApplyModel = 0;
    constants.WhitePoint = 1;
    ...
    check(vkCreateDescriptorSetLayout(device, &lci, nullptr, &layout));
    ...
    check(vkCreateShaderModule(device, &sci, nullptr, &shader));
    ...
    for (uint32_t pass = 0; pass < 2; ++pass)
    {
        ...
        for (uint32_t b = 0; b < 8; ++b)
        {
            ...
            {
                ii[b - 1] = { sampler, images[(pass ? resolve : encode)[b - 1]].view, VK_IMAGE_LAYOUT_GENERAL };
                writes[b].pImageInfo = &ii[b - 1];
            }
        }
    ...
    check(vkQueueWaitIdle(queue)); // standalone test only
    ...
    std::cout << "PASS: production Vulkan encode/resolve, 1507x847 active inside 1536x864, RGBA preserved, padding "
                 "untouched\n";
    ...
    for (auto& uniform : uniforms)
        std::memcpy(uniform.mapped, &constants, sizeof(constants));
    for (uint32_t pass = 0; pass < 2; ++pass)
    {
    ...
    for (uint32_t sample = 0; sample < 2; ++sample)
    {
        ...
        {
            vkCmdBindDescriptorSets(cmd, VK_PIPELINE_BIND_POINT_COMPUTE, pipelineLayout, 0, 1, &sets[pass % 2], 0,
                                    nullptr);
            vkCmdDispatch(cmd, (width + 7) / 8, (height + 7) / 8, 1);
            memoryBarrier(VK_ACCESS_SHADER_WRITE_BIT,
                          VK_ACCESS_SHADER_READ_BIT | VK_ACCESS_SHADER_WRITE_BIT | VK_ACCESS_TRANSFER_READ_BIT);
    ...
    for (auto b : { uniforms[0], uniforms[1], readback })
    {
