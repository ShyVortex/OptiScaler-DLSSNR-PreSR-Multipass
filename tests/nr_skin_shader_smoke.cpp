...
{
    return std::abs(a.r - b.r) < 0.0001f && std::abs(a.g - b.g) < 0.0001f && std::abs(a.b - b.b) < 0.0001f &&
           a.a == b.a;
}
...
try
{
    ...
    data.pSysMem = edited.data();
    check(device->CreateTexture2D(&desc, &data, &model));
    ...
    check(device->CreateTexture2D(&desc, nullptr, &keep));
    ...
    settings.Passthrough = 1;
    settings.ApplyModel = 1;
    settings.ReversibleMode = 2;
    ...
    auto run = [&]()
    {
        ...
        memcpy(result.data(), mapped.pData, sizeof(result));
        ctx->Unmap(readback.Get(), 0);
        return result;
    };
    auto result = run();
    expect(same(result[0], edited[0]) && same(result[1], edited[1]), "Disabled filter changed output");
    settings.SkinProtection = 1;
    result = run();
    expect(same(result[0], edited[0]) && same(result[1], edited[1]), "Unity settings changed output");
    settings.SkinDetail = settings.SkinColour = settings.EnvironmentDetail = settings.EnvironmentColour = 0;
    result = run();
    expect(same(result[0], base[0]) && same(result[1], base[1]), "Zero strengths did not restore input");
    settings.EnvironmentDetail = settings.EnvironmentColour = 1;
    result = run();
    expect(same(result[0], base[0]) && same(result[1], edited[1]), "Skin/scene separation failed");
    settings.SkinDetail = 1;
    result = run();
    ...
    settings.ShowSkinMask = 1;
    result = run();
    expect(result[0].r > 0.99f && result[1].r < 0.01f, "Preview mismatch");
    settings.ShowSkinMask = 0;
    settings.ApplyModel = 0;
    result = run();
    ...
    settings.Mode = DlssNrMode_EncodeResidual;
    settings.ExposurePreMul = 2.0f;
    auto carrier = run();
    ...
    settings.Mode = DlssNrMode_ApplyResidual;
    result = run();
    ...
    for (const auto pixel : result)
        expect(std::isfinite(pixel.r) && std::isfinite(pixel.g) && std::isfinite(pixel.b) && pixel.r >= 0 &&
                   pixel.g >= 0 && pixel.b >= 0,
               "Carrier overshoot produced invalid output");
    settings.Mode = DlssNrMode_UnitExposure;
    result = run();
    ...
    for (auto& pixel : result)
        for (float* channel : { &pixel.r, &pixel.g, &pixel.b })
            *channel =
                DirectX::PackedVector::XMConvertHalfToFloat(DirectX::PackedVector::XMConvertFloatToHalf(*channel));
    ctx->UpdateSubresource(model.Get(), 0, nullptr, result.data(), sizeof(result), 0);
    settings.Mode = DlssNrMode_Resolve;
    result = run();
    for (int i = 0; i < 2; ++i)
        expect(std::abs(result[i].r - darkEdit[i].r) < .00001f && std::abs(result[i].g - darkEdit[i].g) < .00001f &&
